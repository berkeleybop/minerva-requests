...
var request_set = function(user_token, model_id, reasoner_p, group_ids){
    ...
    // See the _user_id for more context.
    anchor._client_id = null;
    ...
    anchor._referring_model_id = null;
     ...
     * @returns {String|null} string or null
     */
    anchor.referring_model_id = function(rmid){

	if( us.isString(rmid) ){
	    anchor._referring_model_id = rmid;
	}

	return anchor._referring_model_id;
    };
     ...
     * @returns {String|null} string or null
     */
    anchor.client_id = function(cid){

	if( us.isString(cid) ){
	    anchor._client_id = cid;
	}

	return anchor._client_id;
    };
	...
	// Only add client id iff we tried to use it.
	if( us.isString(anchor.client_id()) ){
	    rset['client-id'] = anchor.client_id();
	}
	
	// Only add user id iff we tried to use it.
	if( us.isString(anchor.user_id()) ){
	    rset['user-id'] = anchor.user_id();
	}
	
	// Only add user id iff we tried to use it.
	if( us.isString(anchor.referring_model_id()) ){
	    rset['model-id'] = anchor.referring_model_id();
	}
